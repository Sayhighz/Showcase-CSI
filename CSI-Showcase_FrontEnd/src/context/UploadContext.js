/**
 * Context สำหรับจัดการการอัพโหลด
 * ใช้สำหรับจัดการและแชร์ state ที่เกี่ยวข้องกับการอัพโหลดไฟล์ระหว่าง component ต่างๆ
 */
...
export const UploadProvider = ({ children }) => {
  ...
  // ดึงข้อมูลสถานะพื้นที่จัดเก็บไฟล์เมื่อ component mount
  useEffect(() => {
    fetchStorageStatus();
  }, [fetchStorageStatus]);
...
/**
 * Custom hook สำหรับใช้งาน UploadContext
...
export const useUploadContext = () => {
  ...
  if (!context) {
    throw new Error('useUploadContext ต้องใช้ภายใน UploadProvider');
  }
  
  return context;
};

export default { UploadContext, UploadProvider, useUploadContext };