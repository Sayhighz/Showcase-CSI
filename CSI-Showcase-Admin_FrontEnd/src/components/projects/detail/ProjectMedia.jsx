import React from 'react';
import { Image, Typography, Button } from 'antd';
import { PictureOutlined, YoutubeOutlined, FileOutlined, DownloadOutlined } from '@ant-design/icons';
import { URL } from '../../../constants/apiEndpoints';

const { Title } = Typography;

const ProjectMedia = ({ projectDetails }) => {
  // แสดง YouTube Video Embed
  const renderYoutubeEmbed = (url) => {
    if (!url) return null;

    // Extract YouTube video ID
    const getYoutubeVideoId = (url) => {
      const regExp = /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)([^#&?]*).*/;
      const match = url.match(regExp);
      return (match && match[2].length === 11) ? match[2] : null;
    };

    const videoId = getYoutubeVideoId(url);
    if (!videoId) return null;

    return (
      <div className="w-full my-4">
        <div className="relative pb-[56.25%] h-0 overflow-hidden rounded-lg shadow-md">
          <iframe 
            className="absolute top-0 left-0 w-full h-full"
            src={`https://www.youtube.com/embed/${videoId}`}
            title="YouTube video player"
            frameBorder="0"
            allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
            allowFullScreen
          ></iframe>
        </div>
      </div>
    );
  };

  if (!projectDetails) return null;

  const { type } = projectDetails;
  
  // helper: เลือกรูปหลักจาก coursework.image (รองรับกรณีเป็น array/JSON-string) หรือใช้ images[0]
  const getPrimaryCourseworkImage = (cw) => {
    if (!cw) return null;
    const raw = cw.image;
    if (Array.isArray(raw)) {
      return raw[0] || null;
    }
    if (typeof raw === 'string') {
      const trimmed = raw.trim();
      if (!trimmed) return null;
      if (trimmed.startsWith('[')) {
        try {
          const parsed = JSON.parse(trimmed);
          return Array.isArray(parsed) ? (parsed[0] || null) : trimmed;
        } catch {
          return trimmed;
        }
      }
      return trimmed;
    }
    if (Array.isArray(cw.images) && cw.images.length > 0) {
      return cw.images[0] || null;
    }
    return null;
  };

  if (type === 'coursework' && projectDetails.coursework) {
    // รวบรวมรูปภาพทั้งหมด
    const allImages = [];
    
    // เพิ่มโปสเตอร์ (ถ้ามี)
    if (projectDetails.coursework.poster) {
      allImages.push({
        src: projectDetails.coursework.poster,
        alt: 'โปสเตอร์โครงงาน',
        label: 'โปสเตอร์'
      });
    }
    
    // เพิ่มรูปหลัก (ถ้ามี)
    const primaryImage = getPrimaryCourseworkImage(projectDetails.coursework);
    if (primaryImage) {
      allImages.push({
        src: primaryImage,
        alt: 'รูปหลักโครงงาน',
        label: 'รูปหลัก'
      });
    }
    
    // เพิ่มรูปภาพเพิ่มเติมจาก images array (ถ้ามี)
    if (projectDetails.coursework.images && projectDetails.coursework.images.length > 0) {
      projectDetails.coursework.images.forEach((imagePath, index) => {
        // ตรวจสอบไม่ให้ซ้ำกับที่มีอยู่แล้ว
        if (!allImages.some(img => img.src === imagePath)) {
          allImages.push({
            src: imagePath,
            alt: `รูปภาพประกอบ ${index + 1}`,
            label: `รูปภาพประกอบ ${index + 1}`
          });
        }
      });
    }
    
    // เพิ่มรูปภาพจาก files array (ประเภท image) ถ้ามี
    if (projectDetails.files && projectDetails.files.length > 0) {
      projectDetails.files
        .filter(file => file.file_type === 'image')
        .forEach((file, index) => {
          // ตรวจสอบไม่ให้ซ้ำกับที่มีอยู่แล้ว
          if (!allImages.some(img => img.src === file.file_path)) {
            allImages.push({
              src: file.file_path,
              alt: file.file_name || `รูปภาพเพิ่มเติม ${index + 1}`,
              label: file.file_name || `รูปภาพเพิ่มเติม ${index + 1}`
            });
          }
        });
    }

    return (
      <div className="space-y-4">
        {allImages.length > 0 && (
          <div className="bg-gray-50 rounded-lg p-4">
            <Title level={5} className="mb-3 flex items-center">
              <PictureOutlined className="mr-2" />
              รูปภาพโครงงาน ({allImages.length} รูป)
            </Title>
            
            {allImages.length === 1 ? (
              // แสดงรูปเดียวแบบใหญ่
              <div className="flex justify-center">
                <Image
                  src={`${URL}/${allImages[0].src}`}
                  alt={allImages[0].alt}
                  style={{ maxWidth: '100%', maxHeight: '500px', objectFit: 'contain' }}
                  fallback="data:image/png;base64,"
                />
              </div>
            ) : (
              // แสดงหลายรูปแบบ grid พร้อม Preview Group
              <Image.PreviewGroup>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  {allImages.map((image, index) => (
                    <div key={index} className="text-center">
                      <Image
                        src={`${URL}/${image.src}`}
                        alt={image.alt}
                        width={150}
                        height={150}
                        className="object-cover rounded-lg"
                        fallback="data:image/png;base64,"
                      />
                      <p className="text-xs text-gray-600 mt-1 truncate">{image.label}</p>
                    </div>
                  ))}
                </div>
              </Image.PreviewGroup>
            )}
          </div>
        )}
        
        {/* แสดงวิดีโอ (ถ้ามี) */}
        {projectDetails.coursework.clip_video && renderYoutubeEmbed(projectDetails.coursework.clip_video)}
      </div>
    );
  } else if (type === 'competition' && projectDetails.competition) {
    return (
      <div className="space-y-4">
        {projectDetails.competition.poster && (
          <div className="bg-gray-50 rounded-lg p-4">
            <Title level={5} className="mb-3 flex items-center">
              <PictureOutlined className="mr-2" />
              โปสเตอร์
            </Title>
            <div className="flex justify-center">
              <Image
                src={`${URL}/${projectDetails.competition.poster}`}
                alt="โปสเตอร์การแข่งขัน"
                style={{ maxWidth: '100%', maxHeight: '500px', objectFit: 'contain' }}
                fallback="data:image/png;base64,"
              />
            </div>
          </div>
        )}
      </div>
    );
  } else if (type === 'academic' && projectDetails.academic) {
    return (
      <div className="space-y-4">
        {projectDetails.academic.paper_file && (
          <div className="bg-gray-50 rounded-lg p-4">
            <Title level={5} className="mb-3 flex items-center">
            <FileOutlined className="mr-2" />
              เอกสารงานวิจัย
            </Title>
            <Button
              type="primary"
              icon={<DownloadOutlined />}
              href={`${URL}/${projectDetails.academic.paper_file}`}
              target="_blank"
              rel="noopener noreferrer"
              className="bg-[#90278E] hover:bg-purple-700"
            >
              ดาวน์โหลดเอกสารงานวิจัย
            </Button>
          </div>
        )}
      </div>
    );
  }
  
  return null;
};

export default ProjectMedia;